...
fn filter_terms(
    ancestors_of_lower_terms: BTreeSet<String>,
    descendents_of_upper_terms: BTreeSet<String>,
) -> BTreeSet<String> {
    let mut output_terms: BTreeSet<String> = BTreeSet::new();
...
// Return a BTreeSet of all ancestors of a set of terms
fn get_ancestors(lower_terms: BTreeSet<String>, graph: &IndexedMemoryGraph) -> BTreeSet<String> {
    let mut output_terms: BTreeSet<String> = BTreeSet::new();
...
// Return a BTreeSet of all descendents of a set of terms
fn get_descendents(upper_terms: BTreeSet<String>, graph: &IndexedMemoryGraph) -> BTreeSet<String> {
    let mut output_terms: BTreeSet<String> = BTreeSet::new();
    for purl in upper_terms {
        output_terms.insert(purl.clone());
        let mut desc_set: BTreeSet<String> = BTreeSet::new();
        let mut working_gen: BTreeSet<String> = BTreeSet::new();
        let descendents = graph.children(&purl);
        for d in descendents {
            working_gen.insert(d.to_string());
        }
        while working_gen.len() > 0 {
            let mut next_gen: BTreeSet<String> = BTreeSet::new();
...
// Produce a BTreeSet of desired output classes based on input options
pub fn mireot_terms(
    branch_from: Option<BTreeSet<String>>,
    lower_terms: Option<BTreeSet<String>>,
    upper_terms: Option<BTreeSet<String>>,
    graph: &IndexedMemoryGraph,
) -> BTreeSet<String> {
    let output_terms: BTreeSet<String> = match branch_from {
...
pub fn get_annotations(graph: &IndexedMemoryGraph, terms: &BTreeSet<String>) -> BTreeSet<String> {
    let mut annotation_props: BTreeSet<String> =
